#!/usr/bin/env python3
"""Apply the scoped direct-object literal widening candidate to current Biome main."""
                            ...
                            .map(|value| {
                                if !is_const_asserted {
                                    ...
                                    if let Some(widened) = widened {
                                        return collector.reference_to_owned_data(widened);
                                    }
                                }
                                collector.reference_to_resolved_expression(scope_id, &value)
                            })
...
def main() -> None:
    ...
    if count != 1:
        raise SystemExit(f"current-main direct-object anchor count: {count}")
